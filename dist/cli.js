...
async function main() {
  ...
  switch (command) {
    ...
    case "export": {
      ...
      const agentConfigPath = join3(repoPath, ".agentconfig");
      ...
      for (const selectedFormat of selectedFormats) {
        if (selectedFormat === "all") {
          ...
          switch (selectedFormat) {
            case "copilot":
              exportPath = join3(outputDir, ".github", "copilot-instructions.md");
...
Excluded ${privateRuleCount} private rule(s). Use --include-private to include them.`));
    ...
    case "convert": {
      ...
      const outputDir = values.output || dirname2(inputPath);
      const agentDir = join3(outputDir, ".agent");
      if (existsSync3(agentDir) && !values.overwrite) {
...
function updateGitignoreWithPaths(repoPath, paths) {
  const gitignorePath = join3(repoPath, ".gitignore");
  const patterns = [
    "",
  ...
  if (existsSync3(gitignorePath)) {
    ...
    writeFileSync2(gitignorePath, patterns.trim() + "\n");
  }
}
main().catch((error) => {
